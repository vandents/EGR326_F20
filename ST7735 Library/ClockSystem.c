...
void Clock_Init(enum dcofrequency speed){
  ...
  CS->CTL0 |= 0x00010000;                 // configure for nominal DCO frequency = 3 MHz (default)
  CS->CTL0 |= 0x00400000;                 // enable DCO external resistor mode (must be done with DCORSEL == 1)
  CS->CTL0 = (CS->CTL0&~0x00070000) |       // clear DCORSEL bit field
           speed;                       // configure for nominal DCO frequency = 1.5,3,6, or 12 MHz
  CS->CTL1 = 0x00000200 |                 // configure for ACLK sourced from REFOCLK
...
void Clock_Init32kHz(void){
  ...
  PJ->SEL0 |= 0x03;
  PJ->SEL1 &= ~0x03;                      // configure built-in 32 kHz crystal for LFXT operation
//  PJDIR |= 0x02;                        // make PJ.1 LFXTOUT (unnecessary)
//  PJDIR &= ~0x01;                       // make PJ.0 LFXTIN (unnecessary)
  CS->KEY = 0x695A;                       // unlock CS module for register access
  CS->CTL2 = (CS->CTL2&~0x00000003) |       // clear LFXTDRIVE bit field
  ...
  while(CS->IFG&0x00000001){
    CS->CLRIFG = 0x00000001;              // clear the LFXT oscillator interrupt flag
    Crystalstable = Crystalstable + 1;
    if(Crystalstable > 100000){
      return;                           // time out error
    }
  }
  CS->CTL1 = 0x10000000 |                 // configure for SMCLK divider /2 (necessary for LPM3 and LPM3.5)
  ...
  while((PCM->CTL0&0x00003F00) != 0x00000800){
    CPMwait = CPMwait + 1;
    if(CPMwait >= 500000){
      return;                           // time out error
    }
