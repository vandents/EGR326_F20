...
uint8_t static writecommand(uint8_t c, uint16_t cs_bit) {
  P9->OUT |= 0x03;
  P9->OUT &= ~cs_bit;
...
// Outputs: 8-bit reply
// Assumes: UCA3 and Port 9 have already been initialized and enabled
uint8_t static writedata(uint8_t c, uint16_t cs_bit) {
  P9->OUT |= 0x03;
  P9->OUT &= ~cs_bit;
  while((EUSCI_A3->IFG&0x0002)==0x0000){};    // wait until EUSCI_A3->TXBUF empty
  DC |= DC_BIT;
...
//    parrotdelay(5901);	// 1 msec, tuned at 48 MHz
    parrotdelay(369);	// 1 msec, tuned at 3 MHz
    n--;
  }
}
...
void static commandList(const uint8_t *addr, uint16_t cs_bit) {
  ...
  while(numCommands--) {                 // For each command...
    writecommand(*(addr++), cs_bit);     //   Read, issue command
    numArgs  = *(addr++);                //   Number of args to follow
    ms       = numArgs & DELAY;          //   If hibit set, delay follows args
    numArgs &= ~DELAY;                   //   Mask out delay bit
    while(numArgs--) {                   //   For each argument...
      writedata(*(addr++), cs_bit);      //     Read, issue argument
    }
...
void static commonInit(const uint8_t *cmdList, uint16_t cs_bit) {
  ...
  if(cmdList) commandList(cmdList, cs_bit);
}
...
// Input: none
// Output: none
void ST7735_InitB(uint16_t cs_bit) {
  commonInit(Bcmd, cs_bit);
  ST7735_SetCursor(0, 0);
  StTextColor = ST7735_YELLOW;
  ST7735_FillScreen(0, cs_bit);                 // set screen to black
}
...
// Output: none
void ST7735_InitR(enum initRFlags option, uint16_t cs_bit) {
  commonInit(Rcmd1, cs_bit);
  if(option == INITR_GREENTAB) {
    commandList(Rcmd2green, cs_bit);
    ColStart = 2;
    RowStart = 1;
  } else {
    // colstart, rowstart left at default '0' values
    commandList(Rcmd2red, cs_bit);
  }
  commandList(Rcmd3, cs_bit);

  // if black, change MADCTL color filter
  if (option == INITR_BLACKTAB) {
    writecommand(ST7735_MADCTL, cs_bit);
    writedata(0xC0, cs_bit);
  }
  TabColor = option;
  ST7735_SetCursor(0, 0);
  StTextColor = ST7735_YELLOW;
...
void static setAddrWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint16_t cs_bit) {
  writecommand(ST7735_CASET, cs_bit); // Column addr set
  writedata(0x00, cs_bit);
  writedata(x0+ColStart, cs_bit);     // XSTART
  writedata(0x00, cs_bit);
  writedata(x1+ColStart, cs_bit);     // XEND

  writecommand(ST7735_RASET, cs_bit); // Row addr set
  writedata(0x00, cs_bit);
  writedata(y0+RowStart, cs_bit);     // YSTART
  writedata(0x00, cs_bit);
  writedata(y1+RowStart, cs_bit);     // YEND

  writecommand(ST7735_RAMWR, cs_bit); // write to RAM
}


// Send two bytes of data, most significant byte first
// Requires 2 bytes of transmission
void static pushColor(uint16_t color, uint16_t cs_bit) {
  writedata((uint8_t)(color >> 8), cs_bit);
  writedata((uint8_t)color, cs_bit);
}
...
// Output: none
void ST7735_DrawPixel(int16_t x, int16_t y, uint16_t color, uint16_t cs_bit) {

  if((x < 0) || (x >= _width) || (y < 0) || (y >= _height)) return;

//  setAddrWindow(x,y,x+1,y+1); // original code, bug???
  setAddrWindow(x,y,x,y, cs_bit);

  pushColor(color, cs_bit);
}
...
//        color 16-bit color, which can be produced by ST7735_Color565()
// Output: none
void ST7735_DrawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color, uint16_t cs_bit) {
  uint8_t hi = color >> 8, lo = color;

  // Rudimentary clipping
  if((x >= _width) || (y >= _height)) return;
  if((y+h-1) >= _height) h = _height-y;
  setAddrWindow(x, y, x, y+h-1, cs_bit);
...
//        color 16-bit color, which can be produced by ST7735_Color565()
// Output: none
void ST7735_DrawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color, uint16_t cs_bit) {
  uint8_t hi = color >> 8, lo = color;

  // Rudimentary clipping
  if((x >= _width) || (y >= _height)) return;
  if((x+w-1) >= _width)  w = _width-x;
  setAddrWindow(x, y, x+w-1, y, cs_bit);
...
// Output: none
void ST7735_FillScreen(uint16_t color, uint16_t cs_bit) {
  ST7735_FillRect(0, 0, _width, _height, color, cs_bit);  // original
...
//        color 16-bit color, which can be produced by ST7735_Color565()
// Output: none
void ST7735_FillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color, uint16_t cs_bit) {
  uint8_t hi = color >> 8, lo = color;
  ...
  setAddrWindow(x, y, x+w-1, y+h-1, cs_bit);
...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h, uint16_t cs_bit){
  ...
  setAddrWindow(x, y-h+1, x+w-1, y, cs_bit);

  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
                                        // send the top 8 bits
      writedata((uint8_t)(image[i] >> 8), cs_bit);
                                        // send the bottom 8 bits
      writedata((uint8_t)image[i], cs_bit);
...
// Output: none
void ST7735_DrawCharS(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size, uint16_t cs_bit){
    ...
    for (j = 0; j<8; j++) {
      if (line & 0x1) {
        if (size == 1) // default size
          ST7735_DrawPixel(x+i, y+j, textColor, cs_bit);
        else {  // big size
          ST7735_FillRect(x+(i*size), y+(j*size), size, size, textColor, cs_bit);
        }
      } else if (bgColor != textColor) {
        if (size == 1) // default size
          ST7735_DrawPixel(x+i, y+j, bgColor, cs_bit);
        else {  // big size
          ST7735_FillRect(x+i*size, y+j*size, size, size, bgColor, cs_bit);
        }
      }
      line >>= 1;
    }
...
//        size      number of pixels per character pixel (e.g. size==2 prints each pixel of font as 2x2 square)
// Output: none
void ST7735_DrawChar(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size, uint16_t cs_bit){
  ...
  setAddrWindow(x, y, x+6*size-1, y+8*size-1, cs_bit);
...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor, uint16_t cs_bit){
  uint32_t count = 0;
  if(y>15) return 0;
  while(*pt){
	  //ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLACK, 1);
	  ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLACK, 2, cs_bit);
    pt++;
    x = x+1;
...
// Output: none
// Variable format 1-10 digits with no space before or after
void ST7735_OutUDec(uint32_t n, uint16_t cs_bit){
  Messageindex = 0;
  fillmessage(n);
  Message[Messageindex] = 0; // terminate
  ST7735_DrawString(StX,StY,Message,StTextColor, cs_bit);
  StX = StX+Messageindex;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1, cs_bit);
  }
}
...
// Output: none
void ST7735_SetRotation(uint8_t m, uint16_t cs_bit) {
  ...
  switch (Rotation) {
   case 0:
     if (TabColor == INITR_BLACKTAB) {
       writedata(MADCTL_MX | MADCTL_MY | MADCTL_RGB, cs_bit);
     } else {
       writedata(MADCTL_MX | MADCTL_MY | MADCTL_BGR, cs_bit);
     }
   ...
   case 1:
     if (TabColor == INITR_BLACKTAB) {
       writedata(MADCTL_MY | MADCTL_MV | MADCTL_RGB, cs_bit);
     } else {
       writedata(MADCTL_MY | MADCTL_MV | MADCTL_BGR, cs_bit);
     }
  ...
  case 2:
     if (TabColor == INITR_BLACKTAB) {
       writedata(MADCTL_RGB, cs_bit);
     } else {
       writedata(MADCTL_BGR, cs_bit);
     }
     _width  = ST7735_TFTWIDTH;
     _height = ST7735_TFTHEIGHT;
    break;
   case 3:
     if (TabColor == INITR_BLACKTAB) {
       writedata(MADCTL_MX | MADCTL_MV | MADCTL_RGB, cs_bit);
     } else {
       writedata(MADCTL_MX | MADCTL_MV | MADCTL_BGR, cs_bit);
     }
     _width  = ST7735_TFTHEIGHT;
     _height = ST7735_TFTWIDTH;
     break;
...
// Output: none
void ST7735_InvertDisplay(int i, uint16_t cs_bit) {
  if(i){
    writecommand(ST7735_INVON, cs_bit);
  } else{
    writecommand(ST7735_INVOFF, cs_bit);
  }
}
// graphics routines
...
// Outputs: none
void ST7735_PlotClear(int32_t ymin, int32_t ymax, uint16_t cs_bit){
  ST7735_FillRect(0, 32, 128, 128, ST7735_Color565(228,228,228), cs_bit); // light grey
  if(ymax>ymin){
    Ymax = ymax;
    Ymin = ymin;
    Yrange = ymax-ymin;
...
// Outputs: none
void ST7735_PlotPoint(int32_t y, uint16_t cs_bit){
  int32_t j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  ...
  if(j<32) j = 32;
  if(j>159) j = 159;
  ST7735_DrawPixel(X,   j,   ST7735_BLUE, cs_bit);
  ST7735_DrawPixel(X+1, j,   ST7735_BLUE, cs_bit);
  ST7735_DrawPixel(X,   j+1, ST7735_BLUE, cs_bit);
  ST7735_DrawPixel(X+1, j+1, ST7735_BLUE, cs_bit);
}
...
// Outputs: none
int32_t lastj=0;
void ST7735_PlotLine(int32_t y, uint16_t cs_bit){
  int32_t i, j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  // X goes from 0 to 127
  ...
  if(lastj < j){
    for(i = lastj+1; i<=j ; i++){
      ST7735_DrawPixel(X,   i,   ST7735_BLUE, cs_bit);
      ST7735_DrawPixel(X+1, i,   ST7735_BLUE, cs_bit);
    }
  }else if(lastj > j){
    for(i = j; i<lastj ; i++){
      ST7735_DrawPixel(X,   i,   ST7735_BLUE, cs_bit);
...
// Outputs: none
void ST7735_PlotPoints(int32_t y1,int32_t y2, uint16_t cs_bit){
  int32_t j;
  if(y1<Ymin) y1=Ymin;
  if(y1>Ymax) y1=Ymax;
  // X goes from 0 to 127
  ...
  if(j<32) j = 32;
  if(j>159) j = 159;
  ST7735_DrawPixel(X, j, ST7735_BLACK, cs_bit);
}
...
// Outputs: none
void ST7735_PlotBar(int32_t y, uint16_t cs_bit){
int32_t j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  // X goes from 0 to 127
  ...
  // y=Ymin maps to j=159
  j = 32+(127*(Ymax-y))/Yrange;
  ST7735_DrawFastVLine(X, j, 159-j, ST7735_BLACK, cs_bit);
...
// Outputs: none
void ST7735_PlotdBfs(int32_t y, uint16_t cs_bit){
int32_t j;
  y = y/2; // 0 to 2047
  if(y<0) y=0;
  if(y>511) y=511;
...
// Inputs: none
// Outputs: none
void ST7735_PlotNextErase(uint16_t cs_bit){
  if(X==127){
    X = 0;
  } else{
    X++;
  }
  ST7735_DrawFastVLine(X,32,128,ST7735_Color565(228,228,228), cs_bit);
}
...
// Outputs: none
void ST7735_OutChar(char ch, uint16_t cs_bit){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    StY++; StX=0;
    if(StY>15){
      StY = 0;
    }
    ST7735_DrawString(0,StY,"                     ",StTextColor, cs_bit);
    return;
  }
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_YELLOW,ST7735_BLACK, 1, cs_bit);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1, cs_bit);
...
// outputs: none
void ST7735_OutString(char *ptr, uint16_t cs_bit){
  while(*ptr){
    ST7735_OutChar(*ptr, cs_bit);
    ptr = ptr + 1;
  }
}
...
// Inputs: none
// Outputs: none
void Output_Init(uint16_t cs_bit){
  ST7735_InitR(INITR_REDTAB, cs_bit);
  ST7735_FillScreen(0, cs_bit);                 // set screen to black
}

// Clear display
void Output_Clear(uint16_t cs_bit){ // Clears the display
  ST7735_FillScreen(0, cs_bit);    // set screen to black
}

// Turn off display (low power)
void Output_Off(uint16_t cs_bit){   // Turns off the display
  Output_Clear(cs_bit);  // not implemented
}

// Turn on display
void Output_On(uint16_t cs_bit){ // Turns on the display
  Output_Init(cs_bit);      // reinitialize
}
