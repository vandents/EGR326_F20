...
void RTC_I2C1_Init(void) {
	...
	// P6.4 - 6.5
	P6->SEL0 |= 0x30;
	P6->SEL1 &= ~0x30;			// Pin function to UCB1
	P3->REN |= 0x30;			// Enable pull resistors
	P3->OUT |= 0x30;			// Pull up
	EUSCI_B1->CTLW0 &= ~1;		// Enable UCB1
}

/**
 * @return Current temp on RTC in degrees Fahrenheit
 */
float RTC_Temperature(void) {
	...
	LSB >>= 6;
	LSB &= 0x03;
	return (float) (((float) LSB) * 0.25 + MSB) * 9.0 / 5.0 + 32;
}
